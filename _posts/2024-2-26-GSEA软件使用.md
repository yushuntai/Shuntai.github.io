...
**Collapse dataset to gene symbols**: 如果表达数据集文件中NAME已经与gene sets database中名字一致，选择FALSE，反之选择TRUEPermutationtype:选择置换类型，phenotype或者genesets.每组样本数目大于7个时，建议选择phenotype，否则选择gene sets。<br>
**Chip platform**: 表达数据集为芯片数据时才需要，目的是对ID进行注释转换，如果已经转换好了就不需要了应该也适用于其它需要转换ID的情况，不过事先转换最方便。<br>
