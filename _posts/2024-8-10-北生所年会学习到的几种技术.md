...
Tet诱导系统包含Tet-ON系统(rtTA)和Tet-Off系统(tTA)；在这两个系统中，使用Tet或其衍生物多西环素（强力霉素Dox）控制**转录激活因子tTA或rtTA**与启动子Ptet结合，从而调控下游基因的表达。在基因调控中，tet-off的应用更加广泛，通过是否加入诱导药物（如Tet或Dox等）实现对目的基因诱导表达的目的。
注意：tTA和rtTA都是转录激活因子。
