...
initialize <-  worker(stop_word="https://raw.githubusercontent.com/yushuntai/R-learning/refs/heads/word-cloud/stopwords.txt")   
...
```
将这几个数，形成第一个df，然后再利用一行命令形成第二个df，一行命令形成第三个df，两行命令形成第四个df
