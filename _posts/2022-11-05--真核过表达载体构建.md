...
我们需要保留BamHI和NotI这两个酶切位点，同时我们还在3FLAG和目的基因之间加上GGGS的linker，使3FLAG和目的基因之间总长度为3的倍数，同时使目的基因和poly(A)尾巴之间的碱基数量也为3的倍数，所以我们在NotI酶切位点前面多留了一个G，这样就是3的倍数了，防止目的基因移码突变。同时要注意nFLAG之前是否有Kozak序列（这是陈师兄的载体，已经加上了），如果没有要记得加上。
