...
# 293T细胞内验证ANKRD5-cFLAG与TCTE1-cMYC的蛋白互作
1）6孔板共标记为A-F总共6个孔；<br>
2）取4个1.5mL的EP管，分配如下：
250μL的jetOPTIMUS buffer + 2μg（？μL）的质粒
EP管A： 5 μL的ddH2O
EP管B： 3.8 μL的ANKRD5-cFLAG
EP管C： 4.5μL的TCTE1-cMYC 

800μL的jetOPTIMUS buffer + 3.2μg的TCTE1-cMYC 和3.2μg的ANKRD5-cFLAG
EP管D： 7.2μL的TCTE1-cMYC + 6.1μL的ANKRD5-cFLAG

3）震荡1s，上下颠倒，室温静置2min；
4）EP管A-C分别加入2.5μL 的jetOPTIMUS reagent，EPD加入8μL的jetOPTIMUS reagent；
5）震荡1s，上下颠倒，室温静置10min；
6）6孔板用opti-MEM给细胞换液，然后缓慢加入步骤2中的mix，对应如下：
EP管A-C分别对应加入A-C孔；
EP管D中分别抽出250μL加入孔D，孔E，孔F；
轻轻晃匀，培养箱中放置12h，换完全培养液培养24h；
...
12）800μL的RIPA（normal）重悬beads，平分成四等份，分别与A-2, B-2, C-2，125μL的共转裂解液共孵育，RT 1h；
13）60μL的 flag beads用RIPA（normal）洗3X5min；
14）800μL的RIPA（normal）重悬beads，平分成四等份，分别与A-3, B-3, C-3，125μL的共转裂解液共孵育，RT 30min；
15）用RIPA（normal）洗MYC和flag beads 3X5min；20μL RIPA（normal）重悬，5μL5Xloading煮beads；
16）
上样顺序如下：


![image](https://user-images.githubusercontent.com/61654690/208442087-d913314e-c704-4fd2-85d5-0b21302a5ca6.png)
